...
```
[ws→esp32] {"H":"21","N":21,"D1":2}   ← コマンドはESP32へ到達
[tx->esp]  "...N...21..."              ← TCPへ書き込み済み
[esp32→raw] "{Heartbeat}"             ← ESP32の生出力はHeartbeatのみ
（{21_..} は一度も無し。UNO由来のバイトはゼロ＝断片・ノイズすら無い）
```

**確定**：
- コマンドはブラウザ→中継→ESP32まで届く。
- ESP32が出すのは自分のHeartbeatだけで、**UNOからのバイトが1つも来ていない**（ゴミも無い＝Serial2 RXが完全に無音）。
- → **犯人は中継でもブラウザでも `arduino/` でもESP32ファームのロジックでもない。UNO↔ESP32 のシリアル線、特に戻り（UNO TX D1 → ESP32 GPIO33）が無音。** ソフトでは直らない公算が大。

**消えた仮説**：
- H1/H4/H6（ブラウザ・transport側）→ そもそもESP32が応答を出していないので無関係。
- H2（接続断）→ 切断ログ無し・接続安定。除外。
- H3（取りこぼし）→ "部分受信"ですらない完全無音なので、ほぼ除外。

### 8.1 次の決定打（機材不要）：直結プローブで「行き／戻り」を分離
`ws-bridge` を止め、`tools/esp32-probe.mjs` で 192.168.4.1:100 に直接つなぐ。

```js
// tools/esp32-probe.mjs — ブラウザ/中継を介さず ESP32:100 に直結して往復を試す
import net from "node:net";
const tcp = net.connect(100, "192.168.4.1", () => console.log("[probe] connected"));
tcp.on("data", (c) => console.log("[recv]", JSON.stringify(c.toString("latin1"))));
tcp.on("error", (e) => console.log("[probe] error", e.message));
tcp.on("close", () => console.log("[probe] closed"));
setInterval(() => tcp.write("{Heartbeat}"), 1000);
setTimeout(() => { console.log("[send] 前進"); tcp.write('{"H":"1","N":3,"D1":3,"D2":150}'); }, 2000);
setTimeout(() => { console.log("[send] 停止"); tcp.write('{"H":"1","N":4,"D1":0,"D2":0}'); }, 3800);
setInterval(() => { console.log("[send] 距離"); tcp.write('{"H":"21","N":21,"D1":2}'); }, 2500);
```

**判定**：
| 観測 | 結論 | 次 |
|---|---|---|
| 車輪が回る & `[recv]` に `{21_..}` 無し | 行きOK・**戻り線が死んでいる** | 物理：ESP32挿し直し／GPIO33側の結線確認。ソフトでは直らない |
| 車輪が回らない | 行きの線も死んでいる | ESP32→UNO結線 or UNO未稼働を確認 |
| 両方OK | 別の交絡 | 再調査 |

### 8.2 本番フォールバック（戻り線が物理的にダメでも戦える）
- **USBで自走（距離読取込みで実証済み）＋ WiFiでカメラ** → 確実に動く本命。
- 行きの線が生きていれば **WiFiで手動ドライブ（センサ読取不要）＋カメラ** も可能（自走は不可、手動はOK）。
