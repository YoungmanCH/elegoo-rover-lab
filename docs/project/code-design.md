...
└── src/
    ├── config.ts             # 全パラメータの集約（校正値・桁・URLも。ハードコーディングの唯一の置き場）
    ├── types.ts              # 型＝契約（Sensors/Command/State/Pose/MotionModel/Tick*/Header/RecordingConfig…）
    ├── main.ts               # 合成点(composition root)：部品を組み配線するだけ
    ├── runner.ts             # 制御ループ：read→step→send（onTick で観測を渡す）
    ├── session.ts            # 実機接続のライフサイクル隔離（RobotSession・多重接続ガード／stage5）
    ├── style.css             # ページ全体の HUD スタイル（index.html が <link>／stage10）
    ├── domain/               # 純・判断ロジック
    │   ├── cleaning.ts       # step() 純粋状態機械（掃除の判断）
    │   ├── scan-decision.ts  # 首振りスキャンの判断（stage6）
    │   └── kinematics.ts     # integratePose：移動量→新Pose の運動学カーネル（stage7a）
    ├── protocol/
    │   └── protocol.ts       # Command⇄JSON・レスポンス⇄値（純）＝境界のWeb側実装
    ├── io/                   # 副作用の隔離（RobotIO 抽象＋実装）
    │   ├── robot.ts          # RobotIO インターフェース（依存の境界）
    │   ├── serial-robot.ts   # USB 実装：transport＋protocol
    │   ├── transport.ts      # Web Serial 送受信
    │   └── ws-transport.ts   # WiFi(WS中継) 実装（stage5）
    ├── sim/
    │   ├── model.ts          # 2D姿勢・壁・レイキャスト・首振りセンサ（純）
    │   └── sim-robot.ts      # RobotIO のシム実装
    ├── localization/         # 推測航法＝実機の推定現在地（stage7b）
    │   ├── motion-model.ts   # 指令＋実dt→移動量（校正・純）
    │   └── pose-estimator.ts # estimateStep：commandToDelta→integratePose
    ├── telemetry/            # 軌跡記録（stage7c/d・stage9）
    │   ├── sample.ts         # 観測→TickSample（平坦化＋丸め・純）
    │   ├── session-meta.ts   # sessionId／ヘッダ生成（純）
    │   ├── trajectory.ts     # ヘッダ＋サンプル列の集約
    │   ├── serialize.ts      # NDJSON / CSV 整形（純）
    │   ├── download.ts       # 保存（Blob DL）＋ファイル名
    │   ├── pose-source.ts    # 真値(Sim)/推定(Estimator) の差し替え
    │   ├── recorder.ts       # onTick→sample→積む（注入でテスト可）
    │   └── recording-session.ts # 記録の寿命・状態・直列化を所有（factory・注入／stage9）
    ├── camera/               # カメラ録画のブラウザ側（stage8c）
    │   ├── stream-url.ts     # ライブ表示URL選択（proxy/direct・純）
    │   ├── control-url.ts    # 録画制御URL組立（純）
    │   └── recorder-client.ts # rec/start・rec/stop（fetch＝副作用）
    └── ui/                   # 描画系（canvas／stage10-11）
        ├── draw.ts           # 戦術マップ描画オーケストレータ
        ├── geometry.ts       # 角度・投影の純幾何（テスト）
        ├── readout.ts        # 数値リードアウト整形（純・テスト）
        └── theme.ts          # 配色・寸法の定数
```
> 別ランタイムの **`tools/`**（Node・カメラ）も併設：`cam-proxy.mjs`（MJPEG 中継＋ffmpeg 録画）／`cam-config.mjs`（設定の定数・top直下）／`lib/`（`mjpeg-demux`/`ffmpeg-args`/`multipart`/`recording-paths`/`ffmpeg-recorder`＝純テスト済み）／`ws-bridge.mjs`（WS↔ESP32 中継）。詳細は [stage8a-c](stage8a-mjpeg-demux.md)。
...
| `session.ts` | 実機接続の**寿命隔離**（RobotSession・差し替え／stage5） | ✗ | ✓(fake注入) |
| `domain/scan-decision.ts` | 首振りスキャンの**判断だけ**（stage6） | ✓ | ✓ |
| `domain/kinematics.ts` | 移動量→新Pose の**運動学だけ**（壁を知らない／stage7a） | ✓ | ✓ |
| `io/ws-transport.ts` | WiFi(WS中継) の**送受信だけ**（副作用隔離／stage5） | ✗ | 手動 |
| `localization/motion-model.ts`・`pose-estimator.ts` | 校正＋**推測航法**で推定Poseを出す（純／stage7b） | ✓ | ✓ |
| `telemetry/sample`・`session-meta`・`trajectory`・`serialize`・`download` | 軌跡の**純部品**（整形・集約・直列化・保存名／stage7c） | ✓(純部分) | ✓ |
| `telemetry/pose-source.ts`・`recorder.ts` | 真値/推定の差替＋**onTick記録**（注入でテスト／stage7d） | ✗(注入で隔離) | ✓ |
| `camera/stream-url.ts`・`control-url.ts` | カメラURLの**選択/組立だけ**（純／stage8c） | ✓ | ✓ |
| `camera/recorder-client.ts` | 録画 start/stop の**fetch だけ**（副作用） | ✗ | smoke |
| `main.ts` | **合成点**：具象を組んで配線するだけ | ✗ | smoke |
| `telemetry/recording-session.ts` | 記録セッションの**寿命・状態・直列化を束ねるだけ**（now/download 注入・factory） | ✗（注入で隔離） | ✓ |
| `ui/draw.ts` | canvas 描画の**オーケストレータ**（各レイヤを呼ぶだけ・状態なし） | ✗ | smoke |
| `ui/geometry.ts`・`ui/readout.ts` | 角度・投影・整形の**純ロジック**（描画から剥がしてテスト） | ✓ | ✓ |
| `ui/theme.ts` | 配色・寸法の**定数**（magic 値の唯一の置き場） | ✓ | — |

### 依存関係（依存逆転）
brain（`cleaning`/`runner`）は具体(serial/sim)でなく **`RobotIO` インターフェース**に依存 → 実機なしでシム検証、同じ brain を実機へ。

```
   ui/draw.ts ──▶ runner.ts ──▶ cleaning.ts(純) ──▶ config.ts(純), types.ts
                  │ （interfaceに依存）
                  ▼
               io/robot.ts «RobotIO»
                  ▲                ▲
...
記録系の配線（stage7d/9）：`main` が合成点として具象を選び、記録方針は `recording-session` に隔離する。
```
   main.ts ──▶ recording-session.ts ──▶ recorder / serialize / session-meta / trajectory / download
       │  （now/download を注入）
       └─────▶ pose-source.ts «PoseSource»   （sim=真値 / 実機=推定 を main が選択して渡す）
                  ▲                ▲
            SimPoseSource    EstimatorPoseSource
```
`recording-session` は now/download を注入で受ける＝DOM/実 Date 無しでユニットテスト可（[stage9](stage9-main-single-responsibility.md)）。
...
2. `sim/model.ts` ＋テスト → `sim-robot.ts` → `runner.ts` → `ui/draw.ts`（**シム上で掃除が動く**）
...
## 9. 現状 / 次の一手
- ✅ 段階1〜4：`config`/`types`/`cleaning`/`sim`/`runner`/`protocol`/`serial-robot`/`ui`。自走（壁検知＋タイマー旋回）＋緊急停止。
- ✅ `arduino/SmartRobotCarV4.0_DIY/`：純正コピー＋**MPU6050初期化バグ修正**（N=24 不採用）。
- ✅ 段階5 ワイヤレス：`tools/ws-bridge.mjs`＋`io/ws-transport.ts`＋`session.ts`（多重接続ガード `RobotSession`）。カメラは `<img>` 表示。
- ✅ 段階6 自律強化：減速・後退・**首振りスキャン**（`domain/scan-decision.ts`／`sim/model` の servo・首振りセンサ）。
- ✅ 段階7 軌跡記録（推定）：`domain/kinematics`（運動学）／`localization/`（推測航法）／`telemetry/`（記録・NDJSON/CSV保存・トレイル描画）。純モジュールは Vitest 緑。
- ✅ 段階10/11 UI：Iron Man 風 HUD（`style.css`）＋ sim 戦術マップ（`ui/` に分割：draw/geometry/readout/theme・幾何/整形は純テスト）。
- 🚧 段階8 カメラ録画：`tools/`（`cam-proxy`＋`lib/` 純部品）実装・node:test 緑。ブラウザ側 `camera/`＋main 配線は仕上げ中。
- 🚧 段階9 main 単一責務化：`telemetry/recording-session.ts`（factory）へ記録を隔離中（テスト先行＝一部 RED）。
- ⚠️ 上記 段階7〜11 は**作業ツリーに未コミット**（`feature/stage7-trajectory-recording`）。
- 次：recording-session 実装完了→main を痩せ版へ／段階8c ブラウザ配線→実機 smoke。
