...
- **頭脳も後退対応済み（stage6b）**：`Command.kind` に `reverse` を追加し、`forward / reverse / rotateLeft / rotateRight / stop` の5種に。詰まり時に後退して脱出する（[research-roomba-speed-and-motion §3](research-roomba-speed-and-motion.md)）。
...
- **ライブ表示は“見るだけ”**。**録画は stage8 で追加中**（localhost プロキシ＋ffmpeg で `<sessionId>.mp4` 保存・軌跡と同じ sessionId で対に。ブラウザ配線は仕上げ中）。**フレームを画像処理に使う（視覚自律）・緑レーザー重畳は未実装**（roadmap論点3/§3.4）。
...
- **自己位置は“推定”のみ・地図は無い**（エンコーダ無し）。推測航法（stage7）で推定し軌跡を NDJSON/CSV 記録できるが、**ドリフトする＝真値ではない**。網羅走行の保証は無い。
- **脱出は単純**：stage6 で**後退**を追加（詰まりから下がる）＋首振りで逃げ場を探すが、袋小路からの確実脱出は保証しない。
- **yaw不使用**。旋回はタイマ依存＝床/電圧でばらつく（毎回ぴったり90度ではない）。
- **超音波は首振りスキャン対応**（stage6：停止時にサーボで左右を振る）。ただし**走行中は概ね正面**＝動的な横/斜め障害物は見落としうる。
