...
> **設計の肝**：軌跡の位置は **`sim/model.ts` の運動学を“影（shadow）”として流用した推定**（モデルベース）で出す。**完全な自己位置推定（IMU yaw / 俯瞰ArUco）は本段階の範囲外**＝[design-pose-trajectory-recording.md](../reference/design-pose-trajectory-recording.md) と将来段階に残す。位置は必ず「推定」と明示する。
...
|---|---|---|---|---|
| 1 | **軌跡の位置（推定）** | sim の運動学カーネルを流用した影推定器（指令＋実dt） | 不要 | 無改造 |
...
│  PoseEstimator（影推定: pose を1tick進める）                         │
...
| **PoseEstimator** | 指令＋実dt から Pose を進める影推定器（ドリフトする＝推定） | `localization/pose-estimator.ts` |
...
| **RecordingSession** | `sessionId`＋`t0`。軌跡と動画を束ねる単位 | `telemetry/session-meta.ts` |
...
│   ├── pose-estimator.ts        # 新規・純: estimateStep（影推定の1tick）
...
│   ├── stream-url.test.ts
│   └── recorder-client.ts       # 新規・Infra: 録画開始/停止を proxy へ（URL組立は純でテスト）
├── sim/model.ts                 # 変更: advance() を integratePose＋clamp へ（既存testは緑のまま）
├── types.ts                     # 変更: Pose 移設＋MotionModel/TickSample/Header 等を追加
├── config.ts                    # 変更: motionModel / recording / telemetry を追加（ハードコーディング集約）
├── ui.ts                        # 変更: draw() にトレイル（任意引数）
├── runner.ts                    # 無改造
...
export type MotionModel = {
    driveCmPerSec: number;    // forward(driveSpeed) の実速度[cm/s]。要実測(§9)
...
## 5. Domain：校正と推定（影 dead-reckoning）
...
export function commandToDelta(cmd: Command, dtMs: number, m: MotionModel)
    : { moveCm: number; turnDeg: number } {
    const sec = dtMs / 1000;
    switch (cmd.kind) {
        case "forward":
...
| ID | ケース | 期待 |
|---|---|---|
| M1 | forward, 基準速, dt=1000 | moveCm=driveCmPerSec, turnDeg=0 |
...
```ts
// makeSample — 観測値を TickSample に組むだけ(純)。pose は precision で丸める(config)。
export function makeSample(in: {...}, precision: number): TickSample { ... }
```
...
// sim: 真値を覗くだけ(推定しない)。estimated=false。
...
// 実機: 影推定。estimated=true。pose を内部に持ち estimateStep で進める。
...
export const defaultMotionModel: MotionModel = {
    driveCmPerSec: 22,    // driveSpeed(=80) の実速度。要実測(§10)。目標20〜30cm/s
    reverseCmPerSec: 20,
...
- **校正値**（`driveCmPerSec`/`turnDegPerSec`）は満充電で実測。電圧低下で速度・角速度が落ちる前提。
