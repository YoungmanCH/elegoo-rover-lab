...
| N | 機能 | パラメータ | 戻り値 |
|---|---|---|---|
| 21 | 超音波 | D1=1障害物判定/2距離値 | `{ID_true/false}`（D1=1） または `{ID_<cm>}`（D1=2） |
| 22 | ライン(IR)センサ値 | D1=0左/1中/2右 | `{ID_センサ値}` |
| 23 | 車体が地面から離れたか | — | `{ID_true/false}`（**反転**：接地→`_true`/離地→`_false`） |
...
- **モード切替スイッチ（Upload/Cam）**：UNO の UART（D0/D1, **9600bps**）は **CH340(USB) と ESP32(Serial2: RXD2=33 / TXD2=4) が共有**し、このスイッチでどちらが UART を占有するかを切り替える。**Upload 側＝USB書き込み/USBシリアル**、**Cam(Run) 側＝ESP32（WiFi動作）**。両者は排他で、Cam 側のとき UNO への USB シリアルは使えない。**WiFiでセンサ応答が返らない（`{Heartbeat}` しか来ない）ときは、まずこのスイッチが Cam 側か疑う**（Upload 側だと ESP32↔UNO が物理的に不通）。ボー一致（両側 9600）・ESP32 ファームは双方向中継（`client.print(sendBuff)`）なので、応答が返らない原因はコードでなくこのスイッチであることが多い。
...
| 緑レーザー/ブラシ等の追加 | ○ | 空きピン＝**D11(サーボY)が筆頭**、GPIO直結不可でMOSFET必須。確定設計は [design-green-laser-and-spare-pins.md](design-green-laser-and-spare-pins.md) |
