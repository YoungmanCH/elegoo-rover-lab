...
- **使いどころ**：可視化したいが**純正シリアルプロトコルに取得コマンドが無い**（A-2にN無し）。出すなら**別コマンド（例 `N=25`）を自前追加**する（`N=24`＝Yaw返却は**不採用**＝旋回はタイマー式で yaw を使わないため、電圧用には別 N を使う）。詳細は [code-design §5](../project/code-design.md)。
